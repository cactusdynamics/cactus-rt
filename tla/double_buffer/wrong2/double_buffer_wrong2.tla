...
variables
    ...
    g_idx = [idx |-> 1, newdata |-> FALSE], \* the current index the writer should write to.
        ...
        while reader_outer_loop_idx <= kTotalIterationsCount do
            ...
            ReaderCheckNewData:
                if reader_slot_idx.newdata = TRUE then
                    ReaderSwapIndexAndClearNewData:
                        g_idx := [
                            idx |-> IF reader_slot_idx.idx = 1 THEN 2 ELSE 1,
                            newdata |-> FALSE
                        ];
            ...
            ReaderReadDataLoop:
                while reader_data_read_idx <= kObjectSize do
                    ...
                    assert DataIsNotTorn(g_slots[reader_slot_idx.idx]);
                    ...
                    assert g_slots[reader_slot_idx.idx][reader_data_read_idx] > reader_maximum_data;

                    reader_local_data[reader_data_read_idx] := g_slots[reader_slot_idx.idx][reader_data_read_idx];
...
\* BEGIN TRANSLATION (chksum(pcal) = "189d0cf5" /\ chksum(tla) = "cfd43695")
...
VARIABLES reader_outer_loop_idx, reader_slot_idx, reader_data_read_idx, 
          reader_local_data, reader_maximum_data_tentative, 
          reader_maximum_data, writer_outer_loop_idx, writer_slot_idx, 
          writer_data_write_idx
...
ReaderLoadIndex == /\ pc["reader"] = "ReaderLoadIndex"
                   ...
                   /\ UNCHANGED << g_slots, g_idx, g_writer_maximum_data, 
                                   reader_outer_loop_idx, reader_data_read_idx, 
                                   reader_local_data, 
                                   reader_maximum_data_tentative, 
                                   reader_maximum_data, writer_outer_loop_idx, 
                                   writer_slot_idx, writer_data_write_idx >>
...
ReaderCheckNewData == /\ pc["reader"] = "ReaderCheckNewData"
                      /\ IF reader_slot_idx.newdata = TRUE
                            THEN /\ pc' = [pc EXCEPT !["reader"] = "ReaderSwapIndexAndClearNewData"]
...
ReaderSwapIndexAndClearNewData == /\ pc["reader"] = "ReaderSwapIndexAndClearNewData"
                                  /\ g_idx' =          [
                                                  idx |-> IF reader_slot_idx.idx = 1 THEN 2 ELSE 1,
                                                  newdata |-> FALSE
                                              ]
...
ReaderReadDataLoop == /\ pc["reader"] = "ReaderReadDataLoop"
                      /\ IF reader_data_read_idx <= kObjectSize
                            THEN /\ Assert(DataIsNotTorn(g_slots[reader_slot_idx.idx]), 
                                           "Failure of assertion at line 87, column 21.")
                                 /\ Assert(g_slots[reader_slot_idx.idx][reader_data_read_idx] > reader_maximum_data, 
                                           "Failure of assertion at line 92, column 21.")
                                 /\ reader_local_data' = [reader_local_data EXCEPT ![reader_data_read_idx] = g_slots[reader_slot_idx.idx][reader_data_read_idx]]
                                 /\ reader_data_read_idx' = reader_data_read_idx + 1
                                 /\ pc' = [pc EXCEPT !["reader"] = "ReaderReadDataLoop"]
...
reader == ReaderWaitForWriterStart \/ ReaderOuterLoop \/ ReaderLoadIndex
             \/ ReaderCheckNewData \/ ReaderSwapIndexAndClearNewData
             \/ ReaderReadDataLoop \/ ReaderAfterReadData
...
WriterAfterWriteData == /\ pc["writer"] = "WriterAfterWriteData"
                        /\ g_idx' = [g_idx EXCEPT !.newdata = TRUE]
                        /\ g_writer_maximum_data' = g_writer_maximum_data + 1
...
\* Last modified Wed Apr 03 21:08:02 EDT 2024 by shuhao
