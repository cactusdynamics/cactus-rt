...
class RT1000 : public cactus_rt::CyclicThread, public cactus_rt::ros2::Ros2ThreadMixin {
  ...
  cactus_rt::experimental::Xorshift64Rand rand_engine_{std::random_device{}()};
 ...
 public:
  ...
  void InitializeForRos2() final {
    cmd_vel_sub_ = ros2_adapter_->CreateSubscriptionForLatestValue<Velocity2D, geometry_msgs::msg::Twist>(
      "/cmd_vel", rclcpp::QoS(10)
    );

    feedback_pub_ = ros2_adapter_->CreatePublisher<Velocity2D, geometry_msgs::msg::Twist>(
 ...
 private:
  Velocity2D Drive(double vx, double vy, double w) {
    Velocity2D achieved_vel;
    if (vx != 0.0) {
      achieved_vel.vx = vx + (RandomRealNumber<double>(rand_engine_) - 0.5);
    }

    if (vy != 0.0) {
      achieved_vel.vy = vy + (RandomRealNumber<double>(rand_engine_) - 0.5);
    }

    if (w != 0.0) {
      achieved_vel.w = w + ((RandomRealNumber<double>(rand_engine_) - 0.5) * 0.1);
    }

    // Waste between 100 - 200 us, uniform
    WasteTime(std::chrono::microseconds(static_cast<int64_t>(RandomRealNumber(rand_engine_) * 100.0F + 100.0F)));
