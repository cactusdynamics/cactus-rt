...
TraceAggregator2::TraceAggregator2(TraceAggregatorManager& manager)
    : manager_(manager) {
  this->sticky_trace_packets_.push_back(manager.CreateProcessDescriptorPacket());
}

void TraceAggregator2::Start() {
  std::thread thread{&TraceAggregator2::Run, this};
  thread_.swap(thread);
}

void TraceAggregator2::RequestStop() {
  stop_requested_.store(true, std::memory_order_relaxed);
}

void TraceAggregator2::Join() {
  thread_.join();
}

bool TraceAggregator2::StopRequested() const noexcept {
  return stop_requested_.load(std::memory_order_relaxed);
}

void TraceAggregator2::Run() {
...
void TraceAggregator2::SetupCPUAffinityIfNecessary() const {
  if (manager_.cpu_affinity_.empty()) {
    return;
  }

  cpu_set_t cpuset;
  CPU_ZERO(&cpuset);
  for (auto cpu : manager_.cpu_affinity_) {
    CPU_SET(cpu, &cpuset);
  }
...
TraceAggregatorManager::TraceAggregatorManager(std::string process_name, std::vector<size_t> cpu_affinity)
    : process_name_(process_name),
      cpu_affinity_(cpu_affinity),
      process_track_uuid_(static_cast<uint64_t>(getpid())),
      logger_(quill::create_logger("_trace_aggregator_")) {
}

quill::Logger* TraceAggregatorManager::Logger() {
  return logger_;
}

void TraceAggregatorManager::RegisterSink(std::shared_ptr<Sink> sink) {
  ...
  // TODO: all the initial packets must be written right now.

  sinks_.push_back(sink);
}

void TraceAggregatorManager::DeregisterSink(const Sink* sink) {
  const std::scoped_lock lock(mutex_);
  sinks_.remove_if([sink](const std::shared_ptr<Sink>& s) {
    return sink == s.get();
  });
}

void TraceAggregatorManager::RegisterThreadTracer(std::shared_ptr<ThreadTracer> tracer) {
  if (tracer->tid_ == 0) {
  ...
  // TODO: We need to create a thread track packet and write it to all the known sinks.
}

void TraceAggregatorManager::DeregisterThreadTracer(const ThreadTracer* tracer) {
  const std::scoped_lock lock(mutex_);
  tracers_.remove_if([tracer](const std::shared_ptr<ThreadTracer>& t) {
    return tracer == t.get();
  });
}

void TraceAggregatorManager::Start() {
  const std::scoped_lock lock(mutex_);
  if (trace_aggregator_ == nullptr) {
    trace_aggregator_ = std::make_unique<TraceAggregator2>(*this);
    trace_aggregator_->Start();
  }
}

void TraceAggregatorManager::RequestStop() noexcept {
  const std::scoped_lock lock(mutex_);
  if (trace_aggregator_ != nullptr) {
...
void TraceAggregatorManager::Join() noexcept {
  const std::scoped_lock lock(mutex_);
  if (trace_aggregator_ != nullptr) {
...
Trace TraceAggregatorManager::CreateProcessDescriptorPacket() const {
...
Trace TraceAggregatorManager::CreateThreadDescriptorPacket(const ThreadTracer& thread_tracer) const {
  // NOLINTBEGIN(clang-analyzer-cplusplus.NewDeleteLeaks)
  Trace trace;
...
void TraceAggregator2::AddTrackEventPacketToTrace(
  Trace&                    trace,
