...
class Tracer {
 ...
 public:
  ...
  /**
   * @brief Creates a new thread tracer. Each thread should only have one thread
   * tracer and this method should be called on the thread being traced.
   */
  ...
  void Start() {
    ...
    std::thread thread{&Tracer::Run, this};
    thread_.swap(thread);  // Move ownership to member variable so we don't lose it
  }
